import {FullTrack} from "../../constants/treeStructures.ts";
import {defineTalentTree} from "../../utils/defineTalentTree.ts";

export const explorationTree = {
    talents: defineTalentTree("Exploration", [
        {
            name: "Robust Explorer",
            description: "Increase to base health",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: 10, desc: "+{0} Maximum Health", category: "Health"}],
                [{value: 25, desc: "+{0} Maximum Health", category: "Health"}],
                [{value: 50, desc: "+{0} Maximum Health", category: "Health"}],
            ],
            position: [0, 2.0],
            imageName: "Exploration/Talent_Exploration_HealthyExplorer",
        },
        {
            name: "Cutting It Close",
            description: "Increased movespeed in a drop's final 15 minutes",
            rank: 3,
            prerequisites: ["Field Medic"],
            benefits: [
                [{value: 10, desc: "+{0}% Movement Speed during final moments of the Mission", category: "Time"}],
            ],
            position: [5, 5.0],
            imageName: "Exploration/Talent_Exploration_CuttingItClose",
        },
        {
            name: "Steady Breathing",
            description: "Reduced oxygen consumption rate",
            rank: 1,
            prerequisites: ["Robust Explorer"],
            benefits: [
                [{value: -10, desc: "+{0}% Oxygen Consumption", category: "Upkeep"}],
                [{value: -15, desc: "+{0}% Oxygen Consumption", category: "Upkeep"}],
                [{value: -25, desc: "+{0}% Oxygen Consumption", category: "Upkeep"}],
            ],
            position: [1, 1.0],
            imageName: "Exploration/Talent_Exploration_SteadyBreathing",
        },
        {
            name: "Carbo Loaded",
            description: "Reduced hunger rate",
            rank: 1,
            prerequisites: ["Robust Explorer", "Swift Survivor"],
            benefits: [
                [{value: -10, desc: "+{0}% Food Consumption", category: "Upkeep"}],
                [{value: -15, desc: "+{0}% Food Consumption", category: "Upkeep"}],
                [{value: -25, desc: "+{0}% Food Consumption", category: "Upkeep"}],
            ],
            position: [1, 3.0],
            imageName: "Exploration/Talent_Exploration_CarboLoaded",
        },
        {
            name: "Water Retention",
            description: "Reduced thirst rate",
            rank: 1,
            prerequisites: ["Swift Survivor"],
            benefits: [
                [{value: -10, desc: "+{0}% Water Consumption", category: "Upkeep"}],
                [{value: -15, desc: "+{0}% Water Consumption", category: "Upkeep"}],
                [{value: -25, desc: "+{0}% Water Consumption", category: "Upkeep"}],
            ],
            position: [1, 5.0],
            imageName: "Exploration/Talent_Exploration_WaterRetention",
        },
        {
            name: "Weathering The Storm",
            description: "Increased resistance to exposure",
            rank: 1,
            prerequisites: ["Storm Chaser"],
            benefits: [
                [{value: 5, desc: "+{0}% Exposure Resistance", category: "Exposure"}],
                [{value: 10, desc: "+{0}% Exposure Resistance", category: "Exposure"}],
                [{value: 15, desc: "+{0}% Exposure Resistance", category: "Exposure"}],
            ],
            position: [1, 8.0],
            imageName: "Exploration/Talent_Exploration_WeatheringTheStorm",
        },
        {
            name: "Quick Refresh",
            description: "Increased recovery from exposure",
            rank: 2,
            prerequisites: ["Weathering The Storm"],
            benefits: [
                [{value: 5, desc: "+{0}% Exposure Recovery Rate", category: "Exposure"}],
                [{value: 10, desc: "+{0}% Exposure Recovery Rate", category: "Exposure"}],
                [{value: 15, desc: "+{0}% Exposure Recovery Rate", category: "Exposure"}],
            ],
            position: [3, 8.0],
            imageName: "Exploration/Talent_Exploration_QuickRefresh",
        },
        {
            name: "Field Medic",
            description: "Gives revived players more health",
            rank: 2,
            prerequisites: ["Carbo Loaded", "Water Retention"],
            benefits: [
                [{value: 25, desc: "+{0}% Health Restored to Revived Incapacitated Player", category: "Health"}],
                [{value: 50, desc: "+{0}% Health Restored to Revived Incapacitated Player", category: "Health"}],
            ],
            position: [2, 4.0],
            imageName: "Exploration/Talent_Exploration_FieldMedic",
        },
        {
            name: "And Some For Me",
            description: "Reviving a player also regenerates your health",
            rank: 3,
            prerequisites: ["Field Medic"],
            benefits: [
                [{value: 1, desc: "Trigger Additional Health Regeneration when Reviving a Player", category: "Flag"}],
            ],
            position: [5, 3.0],
            imageName: "Exploration/Talent_Exploration_SomeForMe",
        },
        {
            name: "Plyometrics",
            description: "Reduced jumping stamina cost",
            rank: 1,
            prerequisites: ["Carbo Loaded", "Steady Breathing"],
            benefits: [
                [{value: -10, desc: "+{0}% Stamina Consumed by Jumping", category: "Stamina"}],
                [{value: -25, desc: "+{0}% Stamina Consumed by Jumping", category: "Stamina"}],
                [{value: -50, desc: "+{0}% Stamina Consumed by Jumping", category: "Stamina"}],
            ],
            position: [2, 2.0],
            imageName: "Exploration/Talent_Exploration_Plyometrics",
        },
        {
            name: "Cushioned Fall",
            description: "Reduced chance of sprain from fall damage",
            rank: 3,
            prerequisites: ["Three Point Landing"],
            benefits: [
                [{value: 25, desc: "{0}% chance to avoid Sprain when taking Fall Damage", category: "Environmental"}],
                [{value: 50, desc: "{0}% chance to avoid Sprain when taking Fall Damage", category: "Environmental"}],
            ],
            position: [6, 2.0],
            imageName: "Exploration/Talent_Exploration_CushionedFall",
        },
        {
            name: "Three Point Landing",
            description: "Reduced fall damage",
            rank: 2,
            prerequisites: ["Plyometrics"],
            benefits: [
                [{value: 5, desc: "+{0}% Falling Resistance", category: "Resistance"}],
                [{value: 15, desc: "+{0}% Falling Resistance", category: "Resistance"}],
                [{value: 25, desc: "+{0}% Falling Resistance", category: "Resistance"}],
            ],
            position: [4, 2.0],
            imageName: "Exploration/Talent_Exploration_3PointLanding",
        },
        {
            name: "Arctic Master",
            description: "Increased cold resistance and movespeed in Arctic biomes",
            rank: 4,
            prerequisites: ["Pack Lightly"],
            benefits: [
                [{value: 5, desc: "+{0}% Movement Speed in the Arctic", category: "Movement"}, {value: 15, desc: "+{0}% Cold Resistance in the Arctic", category: "Temperature"}],
            ],
            position: [7, 7.5],
            imageName: "Exploration/Talent_Exploration_ArcticMaster",
        },
        {
            name: "Desert Master",
            description: "Decreased thirst and increased movespeed in Desert biomes",
            rank: 4,
            prerequisites: ["Pack Lightly"],
            benefits: [
                [{value: 5, desc: "+{0}% Movement Speed in the Desert", category: "Movement"}, {value: -25, desc: "+{0}% Water Consumed in the Desert", category: "Consumption"}],
            ],
            position: [8, 6.5],
            imageName: "Exploration/Talent_Exploration_DesertMaster",
        },
        {
            name: "Forest Master",
            description: "Increased health regen and movespeed in Forest biomes",
            rank: 4,
            prerequisites: ["Pack Lightly"],
            benefits: [
                [{value: 5, desc: "+{0}% Movement Speed in the Forest", category: "Movement"}, {value: 10, desc: "+{0}% Health Regeneration in the Forest", category: "Health"}],
            ],
            position: [7, 5.5],
            imageName: "Exploration/Talent_Exploration_ForestMaster",
        },
        {
            name: "Strong Bones",
            description: "Increased sprain recovery speed",
            rank: 3,
            prerequisites: ["Cushioned Fall"],
            benefits: [
                [{value: -15, desc: "+{0}% Sprain Duration", category: "Modifiers"}],
                [{value: -25, desc: "+{0}% Sprain Duration", category: "Modifiers"}],
                [{value: -40, desc: "+{0}% Sprain Duration", category: "Modifiers"}],
            ],
            position: [7, 2.0],
            imageName: "Exploration/Talent_Exploration_StrongBones",
        },
        {
            name: "Storm Chaser",
            description: "Increased stamina regen during storms",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: 10, desc: "+{0}% Stamina Regeneration during a Storm", category: "Stamina"}],
                [{value: 20, desc: "+{0}% Stamina Regeneration during a Storm", category: "Stamina"}],
                [{value: 30, desc: "+{0}% Stamina Regeneration during a Storm", category: "Stamina"}],
            ],
            position: [0, 8.0],
            imageName: "Exploration/Talent_Exploration_StormChaser",
        },
        {
            name: "Like It With the Lights Off",
            description: "Hunger, thirst and oxygen drain much more slowly at night",
            rank: 3,
            prerequisites: ["Night Owl"],
            benefits: [
                [{value: -15, desc: "+{0}% Food Consumed at Night", category: "Consumption"}, {value: -15, desc: "+{0}% Oxygen Consumed at Night", category: "Consumption"}, {value: -15, desc: "+{0}% Water Consumed at Night", category: "Consumption"}],
            ],
            position: [5, 7.5],
            imageName: "Exploration/Talent_Exploration_LikeItWithTheLightsOff",
        },
        {
            name: "Makes the Dream Work",
            description: "Increased shared XP from party members",
            rank: 4,
            prerequisites: ["Field Medic"],
            benefits: [
                [{value: 15, desc: "+{0}% Shared Experience Gained", category: "Experience"}],
            ],
            position: [8, 4.0],
            imageName: "Exploration/Talent_Exploration_MakesTheDreamWork",
        },
        {
            name: "Calming Presence",
            description: "Reduces oxygen consumption for allies within 20m",
            rank: 2,
            prerequisites: ["Steady Breathing"],
            benefits: [
                [{value: 1, desc: "Nearby Allies have reduced Oxygen Consumption", category: "Flag"}],
            ],
            position: [3, 1.0],
            imageName: "Exploration/Talent_Exploration_CalmingPresence",
        },
        {
            name: "Night Owl",
            description: "Increased movespeed at night",
            rank: 3,
            prerequisites: ["Pack Lightly"],
            benefits: [
                [{value: 3, desc: "+{0}% Movement Speed at Night", category: "Movement"}],
                [{value: 6, desc: "+{0}% Movement Speed at Night", category: "Movement"}],
            ],
            position: [4, 7.5],
            imageName: "Exploration/Talent_Exploration_NightOwl",
        },
        {
            name: "Go With The Flow",
            description: "Increased swim speed",
            rank: 2,
            prerequisites: ["Pack Lightly"],
            benefits: [
                [{value: 10, desc: "+{0}% Swimming Speed", category: "Movement"}],
                [{value: 15, desc: "+{0}% Swimming Speed", category: "Movement"}],
            ],
            position: [3, 5.5],
            imageName: "Exploration/Talent_Exploration_GoWithTheFlow",
        },
        {
            name: "Amphibian",
            description: "Reduced oxygen usage underwater",
            rank: 1,
            prerequisites: ["Go With The Flow"],
            benefits: [
                [{value: -10, desc: "+{0}% Oxygen Consumed while Swimming", category: "Consumption"}],
            ],
            position: [4, 5.5],
            imageName: "Exploration/Talent_Exploration_Amphibious",
        },
        {
            name: "Swift Survivor",
            description: "Increase to base movespeed",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: 3, desc: "+{0}% Movement Speed", category: "Movement"}],
                [{value: 5, desc: "+{0}% Movement Speed", category: "Movement"}],
                [{value: 10, desc: "+{0}% Movement Speed", category: "Movement"}],
            ],
            position: [0, 4.0],
            imageName: "Exploration/Talent_Exploration_SwiftSurvival",
        },
        {
            name: "Pack Lightly",
            description: "Increased inventory weight capacity",
            rank: 2,
            prerequisites: [],
            benefits: [
                [{value: 2, desc: "+{0}% Weight Capacity", category: "Weight"}],
                [{value: 5, desc: "+{0}% Weight Capacity", category: "Weight"}],
                [{value: 10, desc: "+{0}% Weight Capacity", category: "Weight"}],
            ],
            position: [2, 6.5],
            imageName: "Solo/Talent_Solo_PackHorse",
        },
    ]),
    fullTracks: [
        { start: "Field Medic", path: [[5, 4.0]], end: "Cutting It Close" },
        { start: "Robust Explorer", path: [[0, 1.0]], end: "Steady Breathing" },
        { start: "Robust Explorer", path: [[0, 3.0]], end: "Carbo Loaded" },
        { start: "Swift Survivor", path: [[0, 3.0]], end: "Carbo Loaded" },
        { start: "Swift Survivor", path: [[0, 5.0]], end: "Water Retention" },
        { start: "Storm Chaser", end: "Weathering The Storm" },
        { start: "Weathering The Storm", end: "Quick Refresh" },
        { start: "Carbo Loaded", path: [[2, 3.0]], end: "Field Medic" },
        { start: "Water Retention", path: [[2, 5.0]], end: "Field Medic" },
        { start: "Field Medic", path: [[5, 4.0]], end: "And Some For Me" },
        { start: "Carbo Loaded", path: [[2, 3.0]], end: "Plyometrics" },
        { start: "Steady Breathing", path: [[2, 1.0]], end: "Plyometrics" },
        { start: "Three Point Landing", end: "Cushioned Fall" },
        { start: "Plyometrics", end: "Three Point Landing" },
        { start: "Pack Lightly", path: [[7, 6.5]], end: "Arctic Master" },
        { start: "Pack Lightly", end: "Desert Master" },
        { start: "Pack Lightly", path: [[7, 6.5]], end: "Forest Master" },
        { start: "Cushioned Fall", end: "Strong Bones" },
        { start: "Night Owl", end: "Like It With the Lights Off" },
        { start: "Field Medic", end: "Makes the Dream Work" },
        { start: "Steady Breathing", end: "Calming Presence" },
        { start: "Pack Lightly", path: [[4, 6.5]], end: "Night Owl" },
        { start: "Pack Lightly", path: [[3, 6.5]], end: "Go With The Flow" },
        { start: "Go With The Flow", end: "Amphibian" },
    ] as FullTrack[],
};
