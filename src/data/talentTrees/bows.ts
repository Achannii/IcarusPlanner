import {FullTrack} from "../../constants/treeStructures.ts";
import {defineTalentTree} from "../../utils/defineTalentTree.ts";

export const bowsTree = {
    talents: defineTalentTree("Bows", [
        {
            name: "Bullseye",
            description: "Increased accuracy with bows",
            rank: 2,
            prerequisites: ["Swift Flight"],
            benefits: [
                [{value: 15, desc: "+{0}% Accuracy with Bows/Crossbows", category: "Ranged Weapon"}],
                [{value: 30, desc: "+{0}% Accuracy with Bows/Crossbows", category: "Ranged Weapon"}],
            ],
            position: [2, 7.0],
            imageName: "Bows/Talent_Bow_IncreasedBowAccuracy",
        },
        {
            name: "Trained Bowman",
            description: "Reduced bow durability loss",
            rank: 2,
            prerequisites: ["Bow Quick Draw"],
            benefits: [
                [{value: -10, desc: "+{0}% Wear Rate of Bows/Crossbows", category: "Utility"}],
                [{value: -20, desc: "+{0}% Wear Rate of Bows/Crossbows", category: "Utility"}],
                [{value: -30, desc: "+{0}% Wear Rate of Bows/Crossbows", category: "Utility"}],
            ],
            position: [2, 5.0],
            imageName: "Bows/Talent_Bow_TrainedBowman",
        },
        {
            name: "Quick Load",
            description: "Faster bow reload",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: 5, desc: "+{0}% Reload Speed of Bows/Crossbows", category: "Ranged Weapon"}],
                [{value: 10, desc: "+{0}% Reload Speed of Bows/Crossbows", category: "Ranged Weapon"}],
                [{value: 15, desc: "+{0}% Reload Speed of Bows/Crossbows", category: "Ranged Weapon"}],
            ],
            position: [0, 7.0],
            imageName: "Bows/Talent_Bow_QuickReload",
        },
        {
            name: "Easy Draw",
            description: "Reduced stamina usage when drawing bows",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: -5, desc: "+{0}% Stamina Consumed by Bow Actions", category: "Stamina"}],
                [{value: -10, desc: "+{0}% Stamina Consumed by Bow Actions", category: "Stamina"}],
                [{value: -15, desc: "+{0}% Stamina Consumed by Bow Actions", category: "Stamina"}],
            ],
            position: [0, 5.0],
            imageName: "Bows/Talent_Bow_EasyDraw",
        },
        {
            name: "Bow Quick Draw",
            description: "Increased aim speed with bows",
            rank: 1,
            prerequisites: ["Easy Draw", "More For Less I"],
            benefits: [
                [{value: 5, desc: "+{0}% Aim Speed with Bows/Crossbows", category: "Ranged Weapon"}],
                [{value: 10, desc: "+{0}% Aim Speed with Bows/Crossbows", category: "Ranged Weapon"}],
                [{value: 15, desc: "+{0}% Aim Speed with Bows/Crossbows", category: "Ranged Weapon"}],
            ],
            position: [1, 5.0],
            imageName: "Bows/Talent_Bow_OverThoseMountains",
        },
        {
            name: "Homing Instinct",
            description: "Chance for arrows to find targets more easily",
            rank: 4,
            prerequisites: ["Soft Spot II"],
            benefits: [
                [{value: 1, desc: "Fired Arrows will lock on to Targets", category: "Flag"}, {value: 1, desc: "Fired Crossbow Bolts will lock on to Targets", category: "Flag"}],
            ],
            position: [7, 5.0],
            imageName: "Bows/Talent_Bow_HomingArrows",
        },
        {
            name: "Twice the Fun",
            description: "Chance to fire two arrows",
            rank: 3,
            prerequisites: ["Soft Spot I", "Bullseye"],
            benefits: [
                [{value: 5, desc: "{0}% chance to fire an additional Arrow/Bolt", category: "Ranged Weapon"}],
                [{value: 15, desc: "{0}% chance to fire an additional Arrow/Bolt", category: "Ranged Weapon"}],
            ],
            position: [5, 7.0],
            imageName: "Bows/Talent_Bow_TwiceTheFun",
        },
        {
            name: "Hindering Shot",
            description: "Arrow strikes slow your target",
            rank: 4,
            prerequisites: ["Trained Bowsmith", "Soft Spot I"],
            benefits: [
                [{value: 5, desc: "{0}% chance to Slow on Hit with Bows/Crossbows", category: "Utility"}],
                [{value: 10, desc: "{0}% chance to Slow on Hit with Bows/Crossbows", category: "Utility"}],
                [{value: 15, desc: "{0}% chance to Slow on Hit with Bows/Crossbows", category: "Utility"}],
            ],
            position: [5, 3.0],
            imageName: "Bows/Talent_Bow_HinderingShot",
        },
        {
            name: "Pinning Shot",
            description: "Chance for arrow strikes to immobilize target",
            rank: 4,
            prerequisites: ["Soft Spot II"],
            benefits: [
                [{value: 5, desc: "{0}% chance to Immobilize on Hit with Bows/Crossbows", category: "Flag"}],
            ],
            position: [7, 7.0],
            imageName: "Bows/Talent_Bow_PinningShot",
        },
        {
            name: "Soft Spot I",
            description: "Increased critical hit multiplier on bows",
            rank: 3,
            prerequisites: ["Bullseye", "Trained Bowsmith", "Trained Bowman"],
            benefits: [
                [{value: 5, desc: "+{0}% Critical Damage with Bows/Crossbows", category: "Generic Damage"}],
                [{value: 15, desc: "+{0}% Critical Damage with Bows/Crossbows", category: "Generic Damage"}],
            ],
            position: [4, 5.0],
            imageName: "Bows/Talent_Bow_SoftSpot1",
        },
        {
            name: "Soft Spot II",
            description: "Increased critical hit multiplier on bows",
            rank: 3,
            prerequisites: ["Twice the Fun"],
            benefits: [
                [{value: 10, desc: "+{0}% Critical Damage with Bows/Crossbows", category: "Generic Damage"}],
                [{value: 25, desc: "+{0}% Critical Damage with Bows/Crossbows", category: "Generic Damage"}],
            ],
            position: [6, 7.0],
            imageName: "Bows/Talent_Bow_SoftSpot2",
        },
        {
            name: "Trained Bowsmith",
            description: "Crafted bows have increased durability",
            rank: 2,
            prerequisites: ["Increased Arrow Damage I", "Bow Quick Draw", "More For Less I"],
            benefits: [
                [{value: 15, desc: "+{0}% Maximum Health for Crafted Bows", category: "Utility"}],
            ],
            position: [2, 3.0],
            imageName: "Bows/Talent_Bow_CraftyFletcher1",
        },
        {
            name: "Crafty Fletcher",
            description: "Chance to craft double when crafting arrows",
            rank: 3,
            prerequisites: ["Trained Bowsmith"],
            benefits: [
                [{value: 10, desc: "{0}% chance to Craft an additional Arrow/Bolt", category: "Flag"}],
            ],
            position: [3, 1.0],
            imageName: "Bows/Talent_Bow_CraftyFletcher2",
        },
        {
            name: "Bow Agility",
            description: "Faster movespeed while holding bows",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: 5, desc: "+{0}% Movement Speed with a Bow/Crossbow Equipped", category: "Movement"}],
                [{value: 8, desc: "+{0}% Movement Speed with a Bow/Crossbow Equipped", category: "Movement"}],
                [{value: 10, desc: "+{0}% Movement Speed with a Bow/Crossbow Equipped", category: "Movement"}],
            ],
            position: [0, 1.0],
            imageName: "Bows/Talent_Bow_BowAgility",
        },
        {
            name: "Swift Flight",
            description: "Increased arrow speed",
            rank: 1,
            prerequisites: ["Quick Load"],
            benefits: [
                [{value: 3, desc: "+{0}% Arrow Speed", category: "Combat"}],
                [{value: 5, desc: "+{0}% Arrow Speed", category: "Combat"}],
                [{value: 10, desc: "+{0}% Arrow Speed", category: "Combat"}],
            ],
            position: [1, 7.0],
            imageName: "Bows/Talent_Bow_SwiftFlight",
        },
        {
            name: "Increased Arrow Damage I",
            description: "Adds a bonus to arrow damage",
            rank: 1,
            prerequisites: ["Bow Agility", "More For Less I"],
            benefits: [
                [{value: 2, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged Weapon"}],
                [{value: 5, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged Weapon"}],
                [{value: 10, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged Weapon"}],
            ],
            position: [1, 1.0],
            imageName: "Bows/Talent_Bow_IncreasedArrowDamage1",
        },
        {
            name: "Increased Arrow Damage II",
            description: "Adds a further bonus to arrow damage",
            rank: 2,
            prerequisites: ["Increased Arrow Damage I"],
            benefits: [
                [{value: 2, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged Weapon"}],
                [{value: 5, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged Weapon"}],
                [{value: 10, desc: "+{0}% Damage with Bows/Crossbows", category: "Ranged Weapon"}],
            ],
            position: [2, 1.0],
            imageName: "Bows/Talent_Bow_IncreasedArrowDamage2",
        },
        {
            name: "More For Less II",
            description: "Cheaper bow crafting",
            rank: 3,
            prerequisites: ["Trained Bowsmith"],
            benefits: [
                [{value: -25, desc: "+{0}% Resource Cost of Crafted Bows/Crossbows", category: "Utility"}],
            ],
            position: [4, 1.0],
            imageName: "Bows/Talent_Bow_MoreForLess2",
        },
        {
            name: "More For Less I",
            description: "Cheaper bow crafting",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: -25, desc: "+{0}% Resource Cost of Crafted Bows/Crossbows", category: "Utility"}],
            ],
            position: [0, 3.0],
            imageName: "Bows/Talent_Bow_MoreForLess",
        },
        {
            name: "Wounding Shot",
            description: "Arrow strikes bleed your target over time",
            rank: 4,
            prerequisites: ["Hindering Shot"],
            benefits: [
                [{value: 5, desc: "{0}% chance to Wound on Hit with Bows/Crossbows", category: "Modifiers"}],
                [{value: 10, desc: "{0}% chance to Wound on Hit with Bows/Crossbows", category: "Modifiers"}],
                [{value: 15, desc: "{0}% chance to Wound on Hit with Bows/Crossbows", category: "Modifiers"}],
            ],
            position: [7, 3.0],
            imageName: "Bows/Talent_Bow_WoundingShot",
        },
        {
            name: "Supply and Demand",
            description: "Increased damage from crafted arrows or bolts",
            rank: 4,
            prerequisites: ["Hindering Shot"],
            benefits: [
                [{value: 2, desc: "+{0}% Projectile Damage on crafted ammo", category: "Flag"}],
                [{value: 5, desc: "+{0}% Projectile Damage on crafted ammo", category: "Flag"}],
                [{value: 10, desc: "+{0}% Projectile Damage on crafted ammo", category: "Flag"}],
            ],
            position: [7, 1.0],
            imageName: "Bows/Talent_Bow_SupplyDemand",
        },
    ]),
    fullTracks: [
        { start: "Swift Flight", end: "Bullseye" },
        { start: "Bow Quick Draw", end: "Trained Bowman" },
        { start: "Easy Draw", end: "Bow Quick Draw" },
        { start: "More For Less I", path: [[1, 3.0]], end: "Bow Quick Draw" },
        { start: "Soft Spot II", path: [[6, 5.0]], end: "Homing Instinct" },
        { start: "Soft Spot I", path: [[4, 7.0]], end: "Twice the Fun" },
        { start: "Bullseye", end: "Twice the Fun" },
        { start: "Trained Bowsmith", end: "Hindering Shot" },
        { start: "Soft Spot I", path: [[4, 3.0]], end: "Hindering Shot" },
        { start: "Soft Spot II", end: "Pinning Shot" },
        { start: "Bullseye", path: [[4, 7.0]], end: "Soft Spot I" },
        { start: "Trained Bowsmith", path: [[4, 3.0]], end: "Soft Spot I" },
        { start: "Trained Bowman", end: "Soft Spot I" },
        { start: "Twice the Fun", end: "Soft Spot II" },
        { start: "Increased Arrow Damage I", path: [[1, 3.0]], end: "Trained Bowsmith" },
        { start: "Bow Quick Draw", path: [[1, 3.0]], end: "Trained Bowsmith" },
        { start: "More For Less I", end: "Trained Bowsmith" },
        { start: "Trained Bowsmith", path: [[3, 3.0]], end: "Crafty Fletcher" },
        { start: "Quick Load", end: "Swift Flight" },
        { start: "Bow Agility", end: "Increased Arrow Damage I" },
        { start: "More For Less I", path: [[1, 3.0]], end: "Increased Arrow Damage I" },
        { start: "Increased Arrow Damage I", end: "Increased Arrow Damage II" },
        { start: "Trained Bowsmith", path: [[4, 3.0]], end: "More For Less II" },
        { start: "Hindering Shot", end: "Wounding Shot" },
        { start: "Hindering Shot", path: [[5, 1.0]], end: "Supply and Demand" },
    ] as FullTrack[],
};
