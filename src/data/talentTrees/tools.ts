import {FullTrack} from "../../constants/treeStructures.ts";
import {defineTalentTree} from "../../utils/defineTalentTree.ts";

export const toolsTree = {
    talents: defineTalentTree("Tools", [
        {
            name: "Good As New",
            description: "Chance to restore pickaxe's durability on breaking hit",
            rank: 4,
            prerequisites: ["Strong Pick"],
            benefits: [
                [{value: 25, desc: "{0}% for Pickaxes to Recover Durability Instead of Breaking", category: "Flag"}],
            ],
            position: [6, 1.0],
            imageName: "Tools/Talent_Tools_GoodAsNew",
        },
        {
            name: "Speedy Mining",
            description: "Increased pickaxe swing speed",
            rank: 1,
            prerequisites: ["Picksmith I"],
            benefits: [
                [{value: 5, desc: "+{0}% Attack Speed with Pickaxes", category: "Combat"}],
                [{value: 10, desc: "+{0}% Attack Speed with Pickaxes", category: "Combat"}],
                [{value: 20, desc: "+{0}% Attack Speed with Pickaxes", category: "Combat"}],
            ],
            position: [1, 3.0],
            imageName: "Tools/Talent_Tools_SpeedyPick",
        },
        {
            name: "Miner Damage",
            description: "Increased pickaxe melee damage",
            rank: 1,
            prerequisites: ["Careful Hits II", "Speedy Mining"],
            benefits: [
                [{value: 20, desc: "+{0}% Pickaxe Melee Damage", category: "Melee DMG"}],
                [{value: 40, desc: "+{0}% Pickaxe Melee Damage", category: "Melee DMG"}],
                [{value: 60, desc: "+{0}% Pickaxe Melee Damage", category: "Melee DMG"}],
            ],
            position: [1, 1.0],
            imageName: "Tools/Talent_Tools_MinerDamage",
        },
        {
            name: "Speed Chopper",
            description: "Increased axe swing speed",
            rank: 1,
            prerequisites: ["Discount Axes I", "Hardy Axe Heads I"],
            benefits: [
                [{value: 5, desc: "+{0}% Attack Speed with Axes", category: "Combat"}],
                [{value: 10, desc: "+{0}% Attack Speed with Axes", category: "Combat"}],
                [{value: 20, desc: "+{0}% Attack Speed with Axes", category: "Combat"}],
            ],
            position: [1, 5.0],
            imageName: "Tools/Talent_Tools_SpeedChopper",
        },
        {
            name: "Big Pick",
            description: "Increased pickaxe mining radius",
            rank: 2,
            prerequisites: ["Miner Damage"],
            benefits: [
                [{value: 10, desc: "+{0}% Mining Radius", category: "Mining"}],
                [{value: 20, desc: "+{0}% Mining Radius", category: "Mining"}],
            ],
            position: [2, 1.0],
            imageName: "Tools/Talent_Tools_StrongPick",
        },
        {
            name: "Picksmith II",
            description: "Even cheaper pickaxe crafting",
            rank: 2,
            prerequisites: ["Big Pick"],
            benefits: [
                [{value: -25, desc: "+{0}% Resource Cost of Crafted Pickaxes", category: "Crafting"}],
            ],
            position: [4, 1.0],
            imageName: "Tools/Talent_Tools_Picksmith1",
        },
        {
            name: "Strong Pick",
            description: "Crafted pickaxes have increased durability",
            rank: 3,
            prerequisites: ["Picksmith II"],
            benefits: [
                [{value: 10, desc: "+{0}% Maximum Health for Crafted Pickaxes", category: "Utility"}],
                [{value: 20, desc: "+{0}% Maximum Health for Crafted Pickaxes", category: "Utility"}],
            ],
            position: [5, 1.0],
            imageName: "Tools/Talent_Tools_Picksmith2",
        },
        {
            name: "Miner Endurance I",
            description: "Reduced stamina cost on pickaxes",
            rank: 2,
            prerequisites: ["Speedy Mining"],
            benefits: [
                [{value: -5, desc: "+{0}% Stamina Consumed by Pickaxe Actions", category: "Stamina"}],
                [{value: -10, desc: "+{0}% Stamina Consumed by Pickaxe Actions", category: "Stamina"}],
                [{value: -15, desc: "+{0}% Stamina Consumed by Pickaxe Actions", category: "Stamina"}],
            ],
            position: [2, 3.0],
            imageName: "Tools/Talent_Tools_MinerEndurance",
        },
        {
            name: "Careful Hits II",
            description: "Reduced durability loss on pickaxes",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: -5, desc: "+{0}% Wear Rate of Pickaxes", category: "Item"}],
                [{value: -10, desc: "+{0}% Wear Rate of Pickaxes", category: "Item"}],
                [{value: -15, desc: "+{0}% Wear Rate of Pickaxes", category: "Item"}],
            ],
            position: [0, 1.0],
            imageName: "Tools/Talent_Tools_CarefulHits",
        },
        {
            name: "Berserker",
            description: "Greatly increased combat abilities with axes but increases stamina and durability usage",
            rank: 2,
            prerequisites: ["Discount Axes II"],
            benefits: [
                [{value: 150, desc: "+{0}% Axe Melee Damage", category: "Melee DMG"}, {value: 10, desc: "+{0}% Attack Speed with Axes", category: "Combat"}, {value: 50, desc: "+{0}% Stamina Consumed by Axe Actions", category: "Stamina"}, {value: 100, desc: "+{0}% Wear Rate of Axes", category: "Item"}],
            ],
            position: [4, 5.0],
            imageName: "Tools/Talent_Tools_Tomahawk",
        },
        {
            name: "Hardy Axe Heads I",
            description: "Reduced durability loss on axes",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: -5, desc: "+{0}% Wear Rate of Axes", category: "Item"}],
                [{value: -10, desc: "+{0}% Wear Rate of Axes", category: "Item"}],
                [{value: -15, desc: "+{0}% Wear Rate of Axes", category: "Item"}],
            ],
            position: [0, 7.0],
            imageName: "Tools/Talent_Tools_HardyAxeHeads",
        },
        {
            name: "Extra Sharp",
            description: "Increased felling damage with axes",
            rank: 2,
            prerequisites: ["Speed Chopper"],
            benefits: [
                [{value: 5, desc: "+{0}% Felling Damage", category: "Utility"}],
                [{value: 10, desc: "+{0}% Felling Damage", category: "Utility"}],
                [{value: 25, desc: "+{0}% Felling Damage", category: "Utility"}],
            ],
            position: [2, 7.0],
            imageName: "Tools/Talent_Tools_ExtraSharp",
        },
        {
            name: "Controlled Rage",
            description: "Reduced penalties for Berserking, increased wood from felling",
            rank: 3,
            prerequisites: ["Berserker"],
            benefits: [
                [{value: -50, desc: "+{0}% Wear Rate of Axes", category: "Item"}, {value: -10, desc: "+{0}% Stamina Consumed by Axe Actions", category: "Stamina"}, {value: 30, desc: "+{0}% Yield from Felling Trees", category: "Felling"}],
            ],
            position: [4, 3.0],
            imageName: "Tools/Talent_Tools_OhWaitIneedThat",
        },
        {
            name: "Discount Axes II",
            description: "Cheaper axe crafting",
            rank: 2,
            prerequisites: ["Extra Sharp"],
            benefits: [
                [{value: -25, desc: "+{0}% Resource Cost of Crafted Axes", category: "Crafting"}],
            ],
            position: [4, 7.0],
            imageName: "Tools/Talent_Tools_DiscountAxes3",
        },
        {
            name: "Hard Axe",
            description: "Crafted axes have increased durability",
            rank: 3,
            prerequisites: ["Discount Axes II"],
            benefits: [
                [{value: 10, desc: "+{0}% Maximum Health for Crafted Axes", category: "Utility"}],
                [{value: 25, desc: "+{0}% Maximum Health for Crafted Axes", category: "Utility"}],
            ],
            position: [5, 7.0],
            imageName: "Tools/Talent_Tools_DiscountAxes2",
        },
        {
            name: "Think of All the Fiber",
            description: "Reduced durability loss on sickles",
            rank: 2,
            prerequisites: ["Speed Chopper", "Speedy Mining"],
            benefits: [
                [{value: -25, desc: "+{0}% Wear Rate of Sickles", category: "Item"}],
                [{value: -50, desc: "+{0}% Wear Rate of Sickles", category: "Item"}],
            ],
            position: [3, 4.0],
            imageName: "Tools/Talent_Tools_ThinkOfAllTheFibre",
        },
        {
            name: "Lumberjack Endurance",
            description: "Reduced stamina cost with axes",
            rank: 2,
            prerequisites: ["Speed Chopper"],
            benefits: [
                [{value: -5, desc: "+{0}% Stamina Consumed by Axe Actions", category: "Stamina"}],
                [{value: -10, desc: "+{0}% Stamina Consumed by Axe Actions", category: "Stamina"}],
                [{value: -15, desc: "+{0}% Stamina Consumed by Axe Actions", category: "Stamina"}],
            ],
            position: [2, 5.0],
            imageName: "Tools/Talent_Tools_LumberjackEndurance",
        },
        {
            name: "Picksmith I",
            description: "Cheaper pickaxe crafting",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: -25, desc: "+{0}% Resource Cost of Crafted Pickaxes", category: "Crafting"}],
            ],
            position: [0, 3.0],
            imageName: "Tools/Talent_Tools_Picksmith1",
        },
        {
            name: "Discount Axes I",
            description: "Cheaper axe crafting",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: -25, desc: "+{0}% Resource Cost of Crafted Axes", category: "Crafting"}],
            ],
            position: [0, 5.0],
            imageName: "Tools/Talent_Tools_DiscountAxes1",
        },
        {
            name: "Careful Hits",
            description: "Reduced durability loss on pickaxes",
            rank: 1,
            prerequisites: ["Strong Pick"],
            benefits: [
                [{value: -5, desc: "+{0}% Wear Rate of Pickaxes", category: "Item"}],
                [{value: -10, desc: "+{0}% Wear Rate of Pickaxes", category: "Item"}],
                [{value: -20, desc: "+{0}% Wear Rate of Pickaxes", category: "Item"}],
            ],
            position: [5, 3.0],
            imageName: "Tools/Talent_Tools_CarefulHits",
        },
        {
            name: "Hardy Axe Heads II",
            description: "Reduced durability loss on axes",
            rank: 1,
            prerequisites: ["Hard Axe"],
            benefits: [
                [{value: -5, desc: "+{0}% Wear Rate of Axes", category: "Item"}],
                [{value: -10, desc: "+{0}% Wear Rate of Axes", category: "Item"}],
                [{value: -20, desc: "+{0}% Wear Rate of Axes", category: "Item"}],
            ],
            position: [5, 5.0],
            imageName: "Tools/Talent_Tools_HardyAxeHeads2",
        },
        {
            name: "Miner Endurance II",
            description: "Reduced stamina cost on pickaxes",
            rank: 1,
            prerequisites: ["Careful Hits"],
            benefits: [
                [{value: -5, desc: "+{0}% Stamina Consumed by Pickaxe Actions", category: "Stamina"}],
                [{value: -10, desc: "+{0}% Stamina Consumed by Pickaxe Actions", category: "Stamina"}],
                [{value: -15, desc: "+{0}% Stamina Consumed by Pickaxe Actions", category: "Stamina"}],
            ],
            position: [6, 3.0],
            imageName: "Tools/Talent_Tools_MinerEndurance2",
        },
    ]),
    fullTracks: [
        { start: "Strong Pick", end: "Good As New" },
        { start: "Picksmith I", end: "Speedy Mining" },
        { start: "Careful Hits II", end: "Miner Damage" },
        { start: "Speedy Mining", end: "Miner Damage" },
        { start: "Discount Axes I", end: "Speed Chopper" },
        { start: "Hardy Axe Heads I", path: [[0, 5.0]], end: "Speed Chopper" },
        { start: "Miner Damage", end: "Big Pick" },
        { start: "Big Pick", end: "Picksmith II" },
        { start: "Picksmith II", end: "Strong Pick" },
        { start: "Speedy Mining", end: "Miner Endurance I" },
        { start: "Discount Axes II", end: "Berserker" },
        { start: "Speed Chopper", path: [[1, 7.0]], end: "Extra Sharp" },
        { start: "Berserker", end: "Controlled Rage" },
        { start: "Extra Sharp", end: "Discount Axes II" },
        { start: "Discount Axes II", end: "Hard Axe" },
        { start: "Speed Chopper", path: [[1, 4.0]], end: "Think of All the Fiber" },
        { start: "Speedy Mining", path: [[1, 4.0]], end: "Think of All the Fiber" },
        { start: "Speed Chopper", end: "Lumberjack Endurance" },
        { start: "Strong Pick", end: "Careful Hits" },
        { start: "Hard Axe", end: "Hardy Axe Heads II" },
        { start: "Careful Hits", end: "Miner Endurance II" },
    ] as FullTrack[],
};
