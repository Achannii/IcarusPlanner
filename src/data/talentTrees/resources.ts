import {FullTrack} from "../../constants/treeStructures.ts";
import {defineTalentTree} from "../../utils/defineTalentTree.ts";

export const resourcesTree = {
    talents: defineTalentTree("Resources", [
        {
            name: "Friend Of The Trees",
            description: "Reduced damage from falling trees",
            rank: 4,
            prerequisites: ["Seasoned Logsman"],
            benefits: [
                [{value: 50, desc: "+{0}% Falling Tree Resistance", category: "Resistance"}],
            ],
            position: [5, 3.0],
            imageName: "Exploration/Talent_Exploration_FriendOfTheTrees",
        },
        {
            name: "Rock Star",
            description: "Increased stone from mining",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: 10, desc: "+{0}% Yield from Mining Stone", category: "Mining"}],
                [{value: 15, desc: "+{0}% Yield from Mining Stone", category: "Mining"}],
                [{value: 20, desc: "+{0}% Yield from Mining Stone", category: "Mining"}],
            ],
            position: [0, 5.0],
            imageName: "Resources/Talent_Resources_StoneYield",
        },
        {
            name: "Waste Not",
            description: "Receive stone in addition to resources when mining ore deposits",
            rank: 2,
            prerequisites: ["Rock Star"],
            benefits: [
                [{value: 5, desc: "{0}% chance to find additional Stone while Mining", category: "Mining"}],
                [{value: 8, desc: "{0}% chance to find additional Stone while Mining", category: "Mining"}],
                [{value: 10, desc: "{0}% chance to find additional Stone while Mining", category: "Mining"}],
            ],
            position: [2, 6.0],
            imageName: "Resources/Talent_Resources_WasteNot",
        },
        {
            name: "All The Good Stuff",
            description: "Receive secondary resources from bushes",
            rank: 2,
            prerequisites: ["Skilled Picker"],
            benefits: [
                [{value: 10, desc: "{0}% chance to find additional Resources in Plants", category: "Generic Harvesting"}],
                [{value: 20, desc: "{0}% chance to find additional Resources in Plants", category: "Generic Harvesting"}],
                [{value: 30, desc: "{0}% chance to find additional Resources in Plants", category: "Generic Harvesting"}],
            ],
            position: [1, 3.0],
            imageName: "Resources/Talent_Resources_AllThGoodStuff",
        },
        {
            name: "Lumber Yield",
            description: "Increased wood from felling",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: 5, desc: "+{0}% Yield from Felling Trees", category: "Felling"}],
                [{value: 10, desc: "+{0}% Yield from Felling Trees", category: "Felling"}],
                [{value: 20, desc: "+{0}% Yield from Felling Trees", category: "Felling"}],
            ],
            position: [0, 1.0],
            imageName: "Resources/Talent_Resources_LumberYield",
        },
        {
            name: "Lucky Strike",
            description: "Chance to mine a deposit in one hit",
            rank: 4,
            prerequisites: ["Unburdened", "Iron Miner"],
            benefits: [
                [{value: 1, desc: "+{0}% to Instantly Break Mineral or Ore Deposits while Mining", category: "Flag"}],
            ],
            position: [6, 5.0],
            imageName: "Resources/Talent_Resources_LuckyStrike",
        },
        {
            name: "Exotic Sprinter",
            description: "Increased movespeed with exotics in your inventory",
            rank: 3,
            prerequisites: ["Dense Packing II"],
            benefits: [
                [{value: 5, desc: "+{0}% Movement Speed while Carrying Exotics", category: "Modifiers"}],
                [{value: 8, desc: "+{0}% Movement Speed while Carrying Exotics", category: "Modifiers"}],
                [{value: 10, desc: "+{0}% Movement Speed while Carrying Exotics", category: "Modifiers"}],
            ],
            position: [4, 8.0],
            imageName: "Resources/Talent_Resources_ExoticSprinter",
        },
        {
            name: "Peerless Lumberjack",
            description: "Chance to instantly chop up a tree",
            rank: 4,
            prerequisites: ["Seasoned Logsman"],
            benefits: [
                [{value: 1, desc: "{0}% chance to Instantly Fell Trees", category: "Flag"}],
            ],
            position: [6, 1.0],
            imageName: "Resources/Talent_Resources_OutstandingLumberjack",
        },
        {
            name: "Skilled Picker",
            description: "Increased yield from harvesting by hand",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: 5, desc: "+{0}% Yield from Foraging", category: "Foraging"}],
                [{value: 10, desc: "+{0}% Yield from Foraging", category: "Foraging"}],
                [{value: 15, desc: "+{0}% Yield from Foraging", category: "Foraging"}],
                [{value: 20, desc: "+{0}% Yield from Foraging", category: "Foraging"}],
            ],
            position: [0, 3.0],
            imageName: "Resources/Talent_Resources_SkilledPicker",
        },
        {
            name: "Oxygen Thief",
            description: "Increased yield from oxite nodes",
            rank: 2,
            prerequisites: ["Rock Star"],
            benefits: [
                [{value: 10, desc: "+{0}% Yield from Mining Oxite", category: "Mining"}],
                [{value: 15, desc: "+{0}% Yield from Mining Oxite", category: "Mining"}],
                [{value: 20, desc: "+{0}% Yield from Mining Oxite", category: "Mining"}],
            ],
            position: [2, 4.0],
            imageName: "Resources/Talent_Resources_OxygenThief",
        },
        {
            name: "Movin' Wood",
            description: "Decreases wood weight in your inventory",
            rank: 2,
            prerequisites: ["Wood Breakdown"],
            benefits: [
                [{value: -5, desc: "+{0}% Carry Weight of Wood", category: "Weight"}],
                [{value: -10, desc: "+{0}% Carry Weight of Wood", category: "Weight"}],
                [{value: -15, desc: "+{0}% Carry Weight of Wood", category: "Weight"}],
                [{value: -30, desc: "+{0}% Carry Weight of Wood", category: "Weight"}],
            ],
            position: [3, 2.0],
            imageName: "Resources/Talent_Resources_MobileTimber",
        },
        {
            name: "Slinging Stone",
            description: "Decreases stone weight in your inventory",
            rank: 3,
            prerequisites: ["Rock Star"],
            benefits: [
                [{value: -5, desc: "+{0}% Carry Weight of Stone", category: "Weight"}],
                [{value: -10, desc: "+{0}% Carry Weight of Stone", category: "Weight"}],
                [{value: -15, desc: "+{0}% Carry Weight of Stone", category: "Weight"}],
                [{value: -25, desc: "+{0}% Carry Weight of Stone", category: "Weight"}],
            ],
            position: [3, 5.0],
            imageName: "Resources/Talent_Resources_MobileMasonry",
        },
        {
            name: "Exotic Power",
            description: "Reduces exotics weight in your inventory",
            rank: 2,
            prerequisites: ["Dense Packing I"],
            benefits: [
                [{value: -5, desc: "+{0}% Carry Weight of Exotics", category: "Weight"}],
                [{value: -10, desc: "+{0}% Carry Weight of Exotics", category: "Weight"}],
                [{value: -15, desc: "+{0}% Carry Weight of Exotics", category: "Weight"}],
                [{value: -25, desc: "+{0}% Carry Weight of Exotics", category: "Weight"}],
            ],
            position: [2, 8.0],
            imageName: "Resources/Talent_Resources_ExoticStrongman",
        },
        {
            name: "Iron Miner",
            description: "Increased yield from iron deposits",
            rank: 3,
            prerequisites: ["Slinging Stone"],
            benefits: [
                [{value: 10, desc: "+{0}% Yield from Mining Iron", category: "Mining"}],
                [{value: 15, desc: "+{0}% Yield from Mining Iron", category: "Mining"}],
                [{value: 20, desc: "+{0}% Yield from Mining Iron", category: "Mining"}],
            ],
            position: [4, 6.0],
            imageName: "Resources/Talent_Resources_MetalMiner",
        },
        {
            name: "Unburdened",
            description: "Decreased ore weight in your inventory",
            rank: 3,
            prerequisites: ["Slinging Stone"],
            benefits: [
                [{value: -5, desc: "+{0}% Carry Weight of Ores", category: "Weight"}],
                [{value: -10, desc: "+{0}% Carry Weight of Ores", category: "Weight"}],
                [{value: -15, desc: "+{0}% Carry Weight of Ores", category: "Weight"}],
                [{value: -25, desc: "+{0}% Carry Weight of Ores", category: "Weight"}],
            ],
            position: [4, 4.0],
            imageName: "Resources/Talent_Resources_MetallicLogistics",
        },
        {
            name: "Carry On",
            description: "Reduced encumberance penalty",
            rank: 1,
            prerequisites: ["Dense Packing I"],
            benefits: [
                [{value: -3, desc: "+{0}% Over-Encumberance Penalty", category: "Weight"}],
                [{value: -8, desc: "+{0}% Over-Encumberance Penalty", category: "Weight"}],
                [{value: -15, desc: "+{0}% Over-Encumberance Penalty", category: "Weight"}],
                [{value: -25, desc: "+{0}% Over-Encumberance Penalty", category: "Weight"}],
            ],
            position: [1, 6.0],
            imageName: "Resources/Talent_Resources_StrongBack",
        },
        {
            name: "Dense Packing I",
            description: "Increased inventory weight capacity",
            rank: 1,
            prerequisites: [],
            benefits: [
                [{value: 5, desc: "+{0}% Weight Capacity", category: "Weight"}],
                [{value: 10, desc: "+{0}% Weight Capacity", category: "Weight"}],
                [{value: 20, desc: "+{0}% Weight Capacity", category: "Weight"}],
            ],
            position: [0, 7.0],
            imageName: "Resources/Talent_Resources_DensePacking",
        },
        {
            name: "Seasoned Logsman",
            description: "Chopped wood is automatically added to your inventory",
            rank: 4,
            prerequisites: ["Wood Breakdown"],
            benefits: [
                [{value: 1, desc: "Wood is automatically gathered from chopping trees", category: "Flag"}],
            ],
            position: [5, 1.0],
            imageName: "Resources/Talent_Resources_EfficientLogsman",
        },
        {
            name: "Wood Breakdown",
            description: "Unlocks the ability to turn wood into sticks",
            rank: 2,
            prerequisites: ["Lumber Yield"],
            benefits: [
                [{value: 1, desc: "Unlocks the ability to turn wood into sticks", category: "Flag"}],
            ],
            position: [1, 1.0],
            imageName: "Resources/Talent_Resources_LuckyLumberjack",
        },
        {
            name: "Dense Packing II",
            description: "Increased inventory weight capacity",
            rank: 3,
            prerequisites: ["Dense Packing I"],
            benefits: [
                [{value: 5, desc: "+{0}% Weight Capacity", category: "Weight"}],
                [{value: 10, desc: "+{0}% Weight Capacity", category: "Weight"}],
                [{value: 20, desc: "+{0}% Weight Capacity", category: "Weight"}],
            ],
            position: [3, 7.0],
            imageName: "Resources/Talent_Resources_DensePacking",
        },
    ]),
    fullTracks: [
        { start: "Seasoned Logsman", end: "Friend Of The Trees" },
        { start: "Rock Star", path: [[2, 5.0]], end: "Waste Not" },
        { start: "Skilled Picker", end: "All The Good Stuff" },
        { start: "Unburdened", path: [[6, 4.0]], end: "Lucky Strike" },
        { start: "Iron Miner", path: [[6, 6.0]], end: "Lucky Strike" },
        { start: "Dense Packing II", path: [[3, 8.0]], end: "Exotic Sprinter" },
        { start: "Seasoned Logsman", end: "Peerless Lumberjack" },
        { start: "Rock Star", path: [[2, 5.0]], end: "Oxygen Thief" },
        { start: "Wood Breakdown", path: [[3, 1.0]], end: "Movin' Wood" },
        { start: "Rock Star", end: "Slinging Stone" },
        { start: "Dense Packing I", path: [[2, 7.0]], end: "Exotic Power" },
        { start: "Slinging Stone", path: [[4, 5.0]], end: "Iron Miner" },
        { start: "Slinging Stone", path: [[4, 5.0]], end: "Unburdened" },
        { start: "Dense Packing I", path: [[1, 7.0]], end: "Carry On" },
        { start: "Wood Breakdown", end: "Seasoned Logsman" },
        { start: "Lumber Yield", end: "Wood Breakdown" },
        { start: "Dense Packing I", end: "Dense Packing II" },
    ] as FullTrack[],
};