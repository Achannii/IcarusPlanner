...
export function prerequisiteMet(
    ...
    return prerequisites.some(req => {
        if (typeof req === 'string') {
            return (pointsInTree[req] || 0) > 0 || isAutoOwnedCreatureTalent(treeKey, req);
        } else if (Array.isArray(req)) {
            return req.every(inner => (pointsInTree[inner] || 0) > 0 || isAutoOwnedCreatureTalent(treeKey, inner));
        }
        return false;
    });
}